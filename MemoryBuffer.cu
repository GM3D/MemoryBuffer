...
namespace gm3d{
      ...
      host_buf(NULL), dev_buf(NULL)
  {
    ...
    cudaMemcpy(dev_buf, v.data().get(), size, cudaMemcpyDeviceToDevice);
    dev_current = true;
  }

  template <class T> thrust::device_vector<T> MemoryBuffer<T>::
  to_dev_vector()
  {
    thrust::device_vector<T> v(n);
    check(cudaMemcpy((void *)v.data().get(), 
		     (void *)addr(device, read), 
		     n * sizeof(T),
    		     cudaMemcpyDeviceToDevice));
    return v;
  }

  void instantiate_templates_cu(){
